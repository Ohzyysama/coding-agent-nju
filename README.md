...
| 🎨 Markdown 渲染 | CLI 最终回复用 Markdown 渲染，代码块带语法高亮与边框 |
| 🧭 全局视野 | `list_directory` 递归列出目录树，Agent 先「环顾四周」再动手 |
| 💾 自动备份 | `write_file` 覆盖前自动备份 `.bak`，防止模型幻觉破坏原文件 |
...
| `write_file` | 写入 / 覆盖本地文件（覆盖前自动备份 `.bak`） |
| `execute_command` | 执行终端命令（危险命令需确认；超长输出截断） |
| `list_directory` | 递归列出目录结构，了解项目全貌 |
...
6. **UX 细节**：CLI 最终回复用 `rich.markdown` 渲染（代码语法高亮、排版精美）；system prompt 动态注入运行平台与工作目录，帮助模型写出正确的命令。
7. **安全写入与全局视野**：`write_file` 覆盖前自动备份 `.bak`（防模型幻觉破坏原文件）；`list_directory` 递归列出目录树，赋予 Agent 处理多文件工程的探索能力。
