...
一个基于大模型的**编程智能体（Coding Agent）**，能自主写代码、执行命令、读取报错并修复，支持多轮连续对话、多会话管理与 Web UI。
...
| 🖥 Web UI + CLI | FastAPI + 原生前端（SSE 流式），另有终端 CLI 入口 |
...
左侧可新建 / 切换 / 删除会话；右侧对话窗实时展示 Agent 的执行过程（思考 → 工具调用 → 执行结果 → 最终回复）。
...
支持命令：`/clear` 清空历史、`/history` 查看上下文占用、`exit` 退出。
